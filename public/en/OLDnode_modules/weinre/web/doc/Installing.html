...
</p><p>To install <span class="weinre">weinre</span>, use <tt>npm</tt>,
...
</p><p>It's likely you will want to install <span class="weinre">weinre</span>
...
</p><p>Here's an example of installing from a binary package via URL:

<pre>
sudo npm -g install http://example.com/path/to/apache-cordova-weinre-X.Y.Z-bin.tar.gz
</pre>

</p><p><span class="weinre">weinre</span> is also deployed on
...
</p><p>The binary packages available are:

<ul>
<li><tt>apache-cordova-weinre-{version}-bin.tar.gz</tt>
</li><li><tt>apache-cordova-weinre-{version}-bin.zip</tt>
</li><li><tt>apache-cordova-weinre-{version}-doc.tar.gz</tt>
</li><li><tt>apache-cordova-weinre-{version}-doc.zip</tt>
</li><li><tt>apache-cordova-weinre-{version}-src.tar.gz</tt>
</li><li><tt>apache-cordova-weinre-{version}-src.zip</tt>
</li></ul>

</p><p>The difference between the <tt>.tar.gz</tt> and <tt>.zip</tt>
archives is the archive format only.  When expanded, both archives
produce the same files.

</p><p>The <tt>doc</tt> archives contain the stand-alone HTML documentation
</p><p>The <tt>src</tt> archives contain the ready-to-rebuild source
</p><p>The <tt>bin</tt> archives contain the ready-to-run binaries

</p><p>If the <tt>{version}</tt> contains a moniker like <tt>pre</tt>, this
